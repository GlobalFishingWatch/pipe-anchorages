...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "anchorages_path", help="path to CSV file containing anchorages to assign names to"
    )
    parser.add_argument("destination_path", help="path to write results to")
    parser.add_argument(
        "--config-path", help="path to configuration file.", default=default_config_path
    )
    ...
    with open(args.anchorages_path) as infile, open(args.destination_path, "w") as outfile:
        ...
        writer = csv.DictWriter(
            outfile, reader.fieldnames + ["iso3", "label", "sublabel", "label_source"]
        )
        writer.writeheader()
        for x in reader:
